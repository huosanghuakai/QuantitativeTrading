...
side = 'buy'  # set it to 'buy' for a buy position, 'sell' for a sell position
...
price = None if order_type == 'market' else ((ask_price * 1.1) if side == 'sell' else (bid_price * 0.9))
...
# set stop-loss trigger price to last_price - 1% for a buy position
# set stop-loss trigger price to last_price + 1% for a sell position
stop_loss_trigger_price = (last_price if order_type == 'market' else price) * (0.9 if side == 'buy' else 1.1)

# stop-loss limit price at i.e. -1% from trigger price for a buy position
# stop-loss limit price at i.e. +1% from trigger price for a sell position
stop_loss_limit_price = stop_loss_trigger_price * (1.001 if side == 'buy' else 0.999)

# set take-profit trigger price to last_price + 1% for a buy position
# set take-profit trigger price to last_price - 1% for a sell position
take_profit_trigger_price = (last_price if order_type == 'market' else price) * (1.1 if side == 'buy' else 0.9)

# take-profit limit price at i.e. -1% from trigger price for a buy position
# take-profit limit price at i.e. +1% from trigger price for a sell position
take_profit_limit_price = take_profit_trigger_price * (1.1 if side == 'buy' else 0.9)
...
print('Going to open a', 'buy' if side == 'buy' else 'sell', 'position',
